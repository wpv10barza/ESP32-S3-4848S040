#!/usr/bin/env bash
set -Eeuo pipefail

# BLOCK 6 — flash selected physical firmware and optionally monitor serial.
PROJECT_BASE="${PROJECT_BASE:-${HOME}/project}"
REPO_DIR="${REPO_DIR:-${PROJECT_BASE}/ESP32-S3-4848S040}"
VENV_DIR="${REPO_DIR}/.venv"
PIO_ENV="${PIO_ENV:-panel_4848s040}"
PORT="${PORT:-}"
BAUD="${BAUD:-115200}"
MONITOR="${MONITOR:-1}"

cd "${REPO_DIR}"
[[ -f "${VENV_DIR}/bin/activate" ]] || { echo "[ERROR] Run BLOCK 2 first."; exit 1; }
source "${VENV_DIR}/bin/activate"

detect_port() {
  find /dev -maxdepth 1 -type c \( -name 'ttyACM*' -o -name 'ttyUSB*' \) -print 2>/dev/null | sort | head -n 1
}

if [[ -z "${PORT}" ]]; then PORT="$(detect_port)"; fi
[[ -n "${PORT}" && -e "${PORT}" ]] || {
  echo "[ERROR] ESP32 serial port not found. Use PORT=/dev/ttyACM0 (or /dev/ttyUSB0)."
  pio device list || true
  exit 20
}

[[ -f ".pio/build/${PIO_ENV}/firmware.bin" ]] || {
  echo "[ERROR] Build artifact missing. Run BLOCK 5 first."
  exit 1
}

pio run -e "${PIO_ENV}" -t upload --upload-port "${PORT}"
printf '\n[OK] BLOCK 6 — flashed %s on %s.\n' "${PIO_ENV}" "${PORT}"

if [[ "${MONITOR}" == "1" ]]; then
  pio device monitor --port "${PORT}" --baud "${BAUD}"
fi
